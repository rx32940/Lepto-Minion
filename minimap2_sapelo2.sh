...
for file in $data_path/PolyAExperiment/barcode*/; do
    ...
    echo $barcode
    minimap2 -ax map-ont $data_path/Lepto_ref/Leptospira_interrogans_Copenhageni_CI.fa $data_path/PolyAExperiment/$barcode/$barcode.fastq > $output_path/map_lepto/${barcode}_CI.sam
    minimap2 -ax map-ont $data_path/Lepto_ref/Leptospira_interrogans_Copenhageni_CII.fa $data_path/PolyAExperiment/$barcode/$barcode.fastq > $output_path/map_lepto/${barcode}_CII.sam
